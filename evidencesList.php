...
// Exibir informações
$access = 0;
if ($perfil == 'admin' || $perfil == 'developer' || $perfil == 'certifier'){
    $access = 1;
}
...
if ($access){
    $html = "<table class='table'>
                ...
                <tbody>
    ";
    $query_evidences_list = "SELECT * FROM evidencias";
    $result_evidences_list = mysqli_query($DB_connect, $query_evidences_list);
    while($row = mysqli_fetch_assoc($result_evidences_list)){
        $html .= "<tr>";
        $html .= "<td>".$row['id']."</td>";
...
<script>
    $("#btn_remove").click(function() {
        ...
        $.ajax({
            url: "removeEvidence.php?id="+ids+"&login="+login
        });

...
if ($access){
    $html = "<table class='table'>
                ...
                <tbody>
    ";
    $query_projects_list = "SELECT * FROM usuarios WHERE login_usuario != '".$login."'";
    $result_projects_list = mysqli_query($DB_connect, $query_projects_list);
    while($row = mysqli_fetch_assoc($result_projects_list)){
        $html .= "<tr>";
        $html .= "<td>".$row['id']."</td>";
        $html .= "<td>".$row['login_usuario']."</td>";
        $html .= "<td>".$row['perfil']."</td>";
        $html .= "<td align='left'><input type='checkbox' class='form-check-input remove' value='".$row['id']."'></td>";
        $html .= "</tr>";
    }
    $html .= "</tbody></table>";
    $html .= "<button id='btn_remove' type='button' class='btn btn-secondary'>Remove</button>";
}
else {
    $html = "<p>Access denied! Contact your administrator if something is wrong.</p>";
}
